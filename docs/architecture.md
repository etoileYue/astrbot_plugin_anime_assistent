...
│                    │                   │                   │              │     │
│               ┌────▼────┐      ┌──────▼──────┐     ┌──────▼──┐  ┌───────▼──────┐  │
│               │ SQLite   │      │  Bangumi    │     │ Markdown│  │  Bangumi     │  │
│               │ (本地DB)  │      │  API (远程)  │     │ 文件存储 │  │  HTML (爬取) │  │
│               └─────────┘      └─────────────┘     └─────────┘  └──────────────┘  │
└──────────────────────────────────────────────────────────────────┘
```
...
| Plugin → Bangumi HTML | 插件通过 HTTP 爬取剧集页面，解析 HTML 提取用户评论 |
...
### Bangumi HTML 页面（爬取数据源）

- **职责**：提供剧集层面的用户评论数据，供访谈引擎生成更具体的 LLM 问题
- **访问方式**：通过 `scraper/bangumi.py` 爬取 `https://bgm.tv/ep/{episode_id}` 页面 HTML
- **数据格式**：解析后的结构化评论数据（Comment 对象列表）
- **特点**：无官方 API，依赖 HTML 结构；评论在剧集播出后不再变化，可安全缓存
- **与 API 关系**：`episode_id` 来自 Bangumi API v0 (`GET /v0/episodes`)，数据获取方式不同但互补
- **容错**：爬取失败不影响访谈主流程，降级为无评论上下文的普通问题生成
