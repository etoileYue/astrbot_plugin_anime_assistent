...
AstrBot 提供插件级别的 KV 存储 API（`put_kv_data` / `get_kv_data`）。需要决定运行时状态（OAuth token、last_check_time 等）是存入 AstrBot KV 还是自己管理的 SQLite。
...
- OAuth token 存储在 `users.bangumi_token` 字段，同时可通过 `_conf_schema.json` 配置初始值
