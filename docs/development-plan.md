...
| 4.3 | 实现访谈状态机 | 管理对话状态流转 |
| 4.4 | 实现初始问题生成 | 基于番剧信息生成第一个问题 |
| 4.5 | 实现追问生成 | 基于用户回答生成追问 |
| 4.6 | 实现结束判断 | 用户主动结束 / 达到最大轮数 |
| 4.7 | 实现会话识别 | 区分访谈消息和普通消息 |

### 验证标准

同步进度后，机器人主动问问题。用户回答后，机器人能追问。说"不聊了"后结束。
